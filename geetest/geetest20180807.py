...
class Crack():
    def __init__(self):
        ...
        self.browser = webdriver.Chrome('G:/cuijiahua/chromedriver.exe')
    ...
    def open(self):
        ...
        time.sleep(10)
        bg_path=save_bg(self.browser, bg_path="bg.png", bg_class=bgImgClass)
        time.sleep(5)
        full_bg_path=save_full_bg(self.browser, full_bg_path="fbg.png", full_bg_class=sourceImgClass)
        time.sleep(2)
        #获取滑动的距离
        left=get_offset(full_bg_path, bg_path)
        print("left==",left)
        #根据偏移量模拟轨迹
        track=get_track(left)
        #根据轨迹进行拖拽
        drag_the_ball(self.browser, track)
